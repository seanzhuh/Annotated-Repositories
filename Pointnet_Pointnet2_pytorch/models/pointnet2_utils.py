...
def farthest_point_sample(xyz, npoint):
    ...
    # at first the farthest sampled centroids are randomly generated
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to(device)
    batch_indices = torch.arange(B, dtype=torch.long).to(device)
    for i in range(npoint):
        centroids[:, i] = farthest
        centroid = xyz[batch_indices, farthest, :].view(B, 1, 3)
        # not calculating xyz w.r.t. all previously selected centroids, but only the single previous centroid
        dist = torch.sum((xyz - centroid) ** 2, -1)
        mask = dist < distance
        # distance store the distance of all points to currently selected centroid
        distance[mask] = dist[mask]
        # next loop this farthest point is treated as centroid
...
def query_ball_point(radius, nsample, xyz, new_xyz):
    ...
    Input:
        ...
        new_xyz: query points, [B, S, 3], S is npoint, the number of sampled centroids
    Return:
